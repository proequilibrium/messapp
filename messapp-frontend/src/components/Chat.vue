      ...
      <div class="col-sm-6 offset-3">
        ...
        <div v-if="sessionStarted" id="chat-container" class="card">
          ...
          <div class="card-footer text-muted">
            ...
            <div class="row">
              <div class="col-sm-10">
                <input v-model="message" type="text" placeholder="napis zpravu" />
              </div>
              <div class="col-sm-2">
        ...
        <div v-else>
          ...
          <br />
          <button @click="startChatSession" class="btn linear-gradient btn-lg btn-block">CHATUJME</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    startChatSession () {
      alert('probehnepost')
      $.post('http://192.168.1.76:8008/api/chats/', (data) => {
        alert('Budete presmerovan: ', data.uri)
    ...
    postMessage (event) {
      ...
      $.post(`http://192.168.1.76:8008/api/chats/${this.$route.params.uri}/messages/`, data, (data) => {
    ...
    joinChatSession () {
      ...
      $.ajax({
        url: `http://192.168.1.76:8008/api/chats/${uri}/`,
    ...
    fetchChatSessionHistory () {
      $.get(`http://192.168.1.76:8008/api/chats/${this.$route.params.uri}/messages/`, (data) => {
        this.messages = data.messages
      })
    },

    connectToWebSocket () {
      const websocket = new WebSocket(`ws://192.168.1.76:8081/${this.$route.params.uri}`)
      console.log('pripojuji websocket', this.$route.params.uri)
    ...
    onError (event) {
      alert('An CHAT error occured:', event.data)
    }
  }
}
</script>
